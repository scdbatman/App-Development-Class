...
func whatIsEatenToday(mealTime: String) -> Meal?{
    
    return meals[mealTime]
    
}

if let foodsEaten = whatIsEatenToday(mealTime: "Breakfast"){
    print(foodsEaten)
}else{
    print("invalid meal")
}
...
func checkMealLog()-> [String: Any]{
    if let mealLog = UserDefaults.standard.dictionary(forKey: "meal log"){
        return mealLog
    }else{
        return[:]
    }
